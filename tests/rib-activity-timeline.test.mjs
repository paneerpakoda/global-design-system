...
test('publishes Activity timeline in the catalogue and playground', () => {
  ...
  assert.match(components, /PUBLISHED_COMPONENT_IDS = Object\.freeze\(\['button','calendar','cards','accordions','activity-timeline','avatar','breadcrumbs'\]\)/);
  assert.match(components, /sandbox: 'activity-timeline'/);
  assert.match(sandbox, /['"]activity-timeline['"]:\s*\{/);
  assert.match(sandbox, /PUBLISHED_SANDBOX_IDS = Object\.freeze\(\['button','calendar','cards','accordion','activity-timeline','avatar','breadcrumbs'\]\)/);
