...
test('publishes Accordion and Activity timeline across the catalogue and playground', () => {
  ...
  assert.match(components, /const PUBLISHED_COMPONENT_IDS = Object\.freeze\(\['button','calendar','cards','accordions','activity-timeline','avatar','breadcrumbs'\]\)/);
  ...
  assert.match(sandbox, /const PUBLISHED_SANDBOX_IDS = Object\.freeze\(\['button','calendar','cards','accordion','activity-timeline','avatar','breadcrumbs'\]\)/);
