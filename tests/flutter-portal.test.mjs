...
test('all fourteen core pages use current Dart constructors and stable preview IDs', () => {
  assert.equal(Object.keys(specs).length, 14);
  assert.equal(Object.values(specs).filter(s => s.kind === 'New').length, 8);
  ...
  for (const [id, spec] of Object.entries(specs)) {
    ...
    assert.equal(ids[spec.index], id);
